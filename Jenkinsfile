       ...
       stage("Perform Snyk and Trivy Code Analysis") {
        ...
        stage("Docker Build Stage") {
            ...
            steps {
                script {
                    echo "VERSION=${env.VERSION}"
                    sh """
                    docker build -t java:${env.VERSION} .
                    """
                }
            }
        }
        ...
        stage("perform the image scanning using the dockle and Grype"){
            ...
            steps{
                script{
                    sh """
                    dockle java:${env.VERSION}
                    grype java:${env.VERSION} > grype-image-scan.txt
                    """
                }
            }
        }
        stage("tagging docker container") {
            ...
            steps {
                sh """
                docker tag java:${env.VERSION} mohan14242/java:${env.VERSION}
                """
            }
